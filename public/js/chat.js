...
async function runSetup() {
  ...
  // ── Saved channels (stored locally in the browser) ─────────
  const SAVED_CHANNELS_KEY = 'liveChatSavedChannels';
  const idTypeSelect   = document.getElementById('id-type');
  const idValueInput   = document.getElementById('id-value');
  const savedListEl    = document.getElementById('saved-channels-list');
  const saveLabelInput = document.getElementById('save-channel-label');
  const saveChannelBtn = document.getElementById('save-channel-btn');

  function loadSavedChannels() {
    try {
      const arr = JSON.parse(localStorage.getItem(SAVED_CHANNELS_KEY) || '[]');
      return Array.isArray(arr) ? arr : [];
    } catch { return []; }
  }

  function persistSavedChannels(list) {
    try { localStorage.setItem(SAVED_CHANNELS_KEY, JSON.stringify(list)); } catch { /* storage unavailable */ }
  }
  ...
  function renderSavedChannels() {
    ...
    channels.forEach((ch, i) => {
      ...
      chip.className = 'saved-channel-chip';
      if (idTypeSelect.value === ch.idType && idValueInput.value.trim() === ch.idValue) {
      ...
      remove.addEventListener('click', (e) => {
        e.stopPropagation();
        persistSavedChannels(loadSavedChannels().filter((_, idx) => idx !== i));
        renderSavedChannels();
      });
      chip.appendChild(remove);

      chip.addEventListener('click', () => {
        idTypeSelect.value = ch.idType;
        idValueInput.value = ch.idValue;
        renderSavedChannels();
      });
  ...
  saveChannelBtn.addEventListener('click', () => {
    ...
    if (!idValue) { idValueInput.focus(); return; }
    const label = saveLabelInput.value.trim() || idValue;

    const channels = loadSavedChannels();
    const existing = channels.findIndex(ch => ch.idType === idType && ch.idValue === idValue);
    if (existing >= 0) channels[existing].label = label;
    else channels.push({ label, idType, idValue });

    persistSavedChannels(channels);
    saveLabelInput.value = '';
    renderSavedChannels();
  });

  idTypeSelect.addEventListener('change', renderSavedChannels);
  idValueInput.addEventListener('input', renderSavedChannels);
